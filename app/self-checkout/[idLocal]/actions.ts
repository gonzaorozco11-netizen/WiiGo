...
export async function confirmarPedido(
  ...
  if (idCodigo && idProfesional) {
    ...
    if (errorReferido) throw new Error(friendlyDbError(errorReferido));
  }

  return { idVenta: venta.id_venta, numero: venta.numero as number, total, descuento: descuentoBeneficio };
}
...
export async function cancelarPedidoCliente(idVenta: string) {
  const supabase = getSupabaseServerClient();
